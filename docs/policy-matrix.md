...
| ACP launch profiles | `internal/acpclient` profile store plus plugin templates | Supported with local trust | `ratchet acp client profiles list`, `add`, `install`, `trust`, `remove`, and `verify` manage local reviewed launch specs. Profiles store commands, args, cwd, and env key names only, not secret values. Built-in ACP agents win over profile names, profile names cannot shadow built-ins, and untrusted profiles do not resolve through `--agent` or `profiles verify`. Profile execution remains limited to explicit foreground `exec`, `drain`, `watch`, `compare`, `flow run`, and redacted `profiles verify` commands. | ACP profile store, plugin template, command, watch/drain/compare, profile verify, and flow tests cover trusted and untrusted resolution. |
