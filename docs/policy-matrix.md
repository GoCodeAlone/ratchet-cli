...
Run `ratchet policy matrix` or `ratchet policy matrix --json` for a read-only
CLI view of these supported, partial, explicit-operator, and deferred layers.
...
| ACP launch profiles | `internal/acpclient` profile store plus plugin templates | Supported with local trust | `ratchet acp client profiles list`, `add`, `install`, `trust`, `remove`, and `verify` manage local reviewed launch specs. Profiles store commands, args, cwd, and env key names only, not secret values. Built-in ACP agents win over profile names, profile names cannot shadow built-ins, and untrusted profiles do not resolve through `--agent` or `profiles verify`. Profile execution remains limited to explicit foreground `exec`, `drain`, `watch`, `compare`, `flow run`, and redacted `profiles verify` commands. `profiles verify` reports session id, stop reason, command fingerprint, and response byte count without prompt text, response text, or env values. | ACP profile store, plugin template, command, watch/drain/compare, profile verify, and flow tests cover trusted and untrusted resolution. |
...
| Retro/self-improvement | `internal/retro` and local project evidence routing | Partial | Retro evidence is opt-in. `ratchet retro analyze --evidence <file> [--session ID] [--json]` reports findings, local actions, and upstream instructions without mutating config or opening PRs. `ratchet retro instructions --evidence <file> [--session ID] [--output file]` renders the same routed output as a Markdown handoff; it does not mutate config, contact GitHub, or open PRs. Automatic local mutation and upstream PR creation are disabled unless a future configurable policy enables them. | Retro analyzer, command, Markdown handoff, and config-gating tests. |
...
retro evidence, blackboard export records, flow action output, and policy
