...
| Daemon session export artifacts | Daemon session RPCs and `ratchet sessions export` | Supported local-only | `ratchet sessions export <id> --output <archive.json>` writes daemon session tree metadata, message history, and compaction records to a local JSON bundle with user-only permissions. Bundles may include prompts, responses, summaries, provider/model metadata, local paths, and branch relationships; shared evidence must use counts and filenames, not payloads. | `TestHandleSessionsExportWritesSensitiveBundle`; command tests cover output validation and summary-only terminal output. |
