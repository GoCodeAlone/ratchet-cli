...
| Session history and compaction | Pi stores sessions as JSONL trees, supports `/tree`, `/fork`, `/clone`, `/compact`, branch summaries, and extension-customized compaction. ACPX stores session metadata and turn history. | Partial. ratchet-cli now exposes daemon-backed `ratchet sessions history`, `ratchet sessions clone`, `ratchet sessions fork`, and `ratchet sessions tree` for separate branch sessions. Full Pi-style in-place tree navigation, branch summaries, and compaction records remain deferred. |
...
|---|---|---|
| P1 | Add compaction records and summaries. | Pi and ACPX show that resumable queues, compaction history, and branch summaries are now table-stakes for long-running harnesses; session lineage history/fork/clone/tree is implemented, while compaction remains deferred. |
