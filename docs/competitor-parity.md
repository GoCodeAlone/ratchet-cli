...
| Session history and compaction | Pi stores sessions as JSONL trees, supports `/tree`, `/fork`, `/clone`, `/compact`, branch summaries, and extension-customized compaction. ACPX stores session metadata and turn history. | Partial. ratchet-cli now exposes daemon-backed `ratchet sessions history`, `ratchet sessions clone`, `ratchet sessions fork`, `ratchet sessions tree`, and `ratchet sessions compactions` for separate branch sessions and persisted compaction records. Full Pi-style in-place tree navigation, branch summaries, and pre-compaction archive forks remain deferred. |
...
|---|---|---|
| P1 | Add branch summaries and pre-compaction archive forks. | Pi and ACPX show that resumable queues, compaction history, and branch summaries are now table-stakes for long-running harnesses; session lineage history/fork/clone/tree and compaction records are implemented, while richer branch/archive semantics remain deferred. |
