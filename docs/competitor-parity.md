...
| Hooks and extension points | Claude Code hooks and settings reload across scopes; Pi extensions can intercept lifecycle/tool events and register tools/commands/UI; Codex has lifecycle hook config docs. | Partial. ratchet-cli has workflow/agent extensibility and retro recording, but not a broad runtime extension SDK. Follow-up: define optional extension hooks around session lifecycle, tool execution, and retro reporting without allowing unreviewed local mutation by default; background drain is also deferred until policy ownership and cancellation are designed. |
...
| P1 | Define background drain policy before automatic queue execution. | ACP client queue and explicit drain are supported; automatic drain needs owner/session scope, cancellation, prompt persistence warnings, and audit evidence. |
