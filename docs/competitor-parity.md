...
| P1 | Add persistent trust policy editing only after the policy matrix is documented. | Runtime `/mode` and `/trust` controls are supported; config mutation and persisted grant management need clearer policy boundaries first. |
