...
| Hooks and extension points | Claude Code hooks and settings reload across scopes and documents managed-policy hook controls; Pi extensions can intercept lifecycle/tool events and register tools/commands/UI; Codex has lifecycle hook config docs. | Supported with review/trust for command hooks and installed-plugin reload; broader SDK deferred. ratchet-cli lists project/plugin hooks, skips them until exact descriptor hook trust is recorded with `ratchet hooks trust <hash>`, lets operators inspect with `ratchet hooks list --cwd .`, preserves user hooks as trusted by default for compatibility, fires session/prompt/tool/permission/compact/stop lifecycle hook points, exposes plugin skills to `ratchet skill`, and refreshes installed plugin capabilities with `ratchet plugin reload`. Marketplace update/autoupdate, managed hooks, dynamic workflows/routines, and the TypeScript extension SDK remain deferred to the runtime extension lifecycle plan. |
