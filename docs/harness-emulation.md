...
| import/export archives | Supported | `ratchet acp client sessions export <id> --history raw|summary|both --output <archive.json>` writes summary archives, raw ACPX-compatible JSON-RPC history, or both; `sessions import <archive.json> --session <id>` imports ratchet summary archives and `exported_by:"acpx"` raw history archives. `ratchet acp client sessions events <id>` reports or copies raw ACPX event logs. Raw export fails when no sidecar is available instead of inventing wire history. Archives and event logs may contain prompt/response content and are sensitive local conversation data. |
